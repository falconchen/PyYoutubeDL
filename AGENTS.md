...
- URL 级 AI 总结持久化在 `AI_SUMMARY_DB_PATH` 的 SQLite 中，由 `ai_summary_worker.py` 异步处理。扩展接口 `/api/ai_summaries` 必须使用独立的 `AI_SUMMARY_ACCESS_TOKEN` 和 `X-Yter-AI-Token`，原始字幕不得写入数据库。
