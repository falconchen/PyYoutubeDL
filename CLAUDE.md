...
**Video QR tail (`qr_tail.py`):** when `VIDEO_QR_TAIL.ENABLED` is set, the downloader appends a few seconds of QR-code outro (pointing at the original URL) to finished videos, before they leave `TMP_DIR`. The feature never re-encodes the main video: it probes the file, encodes a matching tail clip (same codec, resolution, frame rate, profile, timescale, audio encoder, and one empty mov_text track per embedded subtitle), then joins them with the concat demuxer and `-c copy`, verifying duration and stream layout before replacing the file. Anything that cannot be joined losslessly — non-H.264 video, unsupported audio, an audio time base that disagrees with the sample rate (HE-AAC), `bin_data` streams, a missing source URL — is logged and skipped, leaving the file untouched. Needs `ffmpeg`/`ffprobe`, plus `segno` and `pillow`.
