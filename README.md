...
正片不会重新编码：只编码这几秒片尾，再用 ffmpeg 的 concat demuxer 以 `-c copy` 拼接，开销相当于重写一遍文件。实测 285MB 的 1080p 视频约 1.6 秒，336MB、带 3 条内嵌字幕的约 3.5 秒；拼接期间临时占用一份与原片等大的磁盘空间。运行环境需要能直接执行 `ffmpeg` 和 `ffprobe`。

片尾必须与正片的编码参数和流布局完全一致才能无损拼接，因此下列情况会跳过并在下载器日志中记录原因，视频保持原样，不影响下载结果：

- 视频编码不是 H.264（例如 AV1、VP9），或像素格式不是 8 位 4:2:0；
